...
def run(options, input_path, expected_path, func_name):
    ...
    # Create sandbox
    loc_db = LocationDB()
    sb = Sandbox_Linux_x86_32(loc_db, input_path, options, globals())
    try:
...
@pytest.mark.parametrize("instr", [
    "adc", "add", "and", "bcd", "bsx", "bt", "btc", "btr", "bts", "cmp", "code16", "conv", "dec", "exceptions", "floats",
    "inc", "jcc", "lea", "loop", "misc", "mul", "neg", "not", "or", "rcl", "rcr", "rol", "ror", "sar", "sbb", "segs",
    "self_modifying_code", "shl", "shld", "shr", "single_step", "sse", "string", "sub", "xchg", "xor"
])
def test(instr, jitter_name):
    func_name = "test_" + instr
    current_path = os.path.dirname(__file__)
    input = os.path.join(current_path, "test-i386")
    expected = os.path.join(current_path, "expected", func_name + ".exp")
    print("Input:", input, "\tInstruction", instr, "\tFunction", func_name)
    print("Expected:", expected)

    options = Sandbox_Linux_x86_32.parser().parse_args(["--jitter", jitter_name])
