...
def check(expr_in, expr_out):
    ...
    if result != z3.unsat:
        print("ERROR: a counter-example has been found:")
        model = solver.model()
        print(model)
...
to_test = [(ExprInt(1, 32) - ExprInt(1, 32), ExprInt(0, 32)),
           ...
           (ExprOp('>>>', ExprOp('<<<', a, ExprInt(10, 32)), ExprInt(2, 32)) ^ ExprOp('>>>',
                                                                                      ExprOp('<<<', a, ExprInt(10, 32)),
                                                                                      ExprInt(2, 32)),
            ExprInt(0, 32)),
           ...
           (ExprCompose(ExprId("a", 8), ExprId("b", 24)) & ExprInt(0xFF, 32),
            ExprCompose(ExprId("a", 8), ExprInt(0x0, 24))),
           (ExprCompose(ExprId("a", 8), ExprInt(0x12, 8), ExprId("b", 16)) & ExprInt(0xFFFF, 32),
            ExprCompose(ExprId("a", 8), ExprInt(0x12, 24))),
           (ExprCompose(ExprId("a", 8), ExprInt(0x1234, 16), ExprId("b", 8)) & ExprInt(0xFFFF, 32),
            ExprCompose(ExprId("a", 8), ExprInt(0x34, 24))),
           ...
           (ExprCond(a, ExprInt(0x10, 32), ExprInt(0x20, 32)) + ExprCond(a, ExprInt(0x13370000, 32),
                                                                         ExprInt(0x13380000, 32)),
...
@pytest.mark.parametrize("e_input,e_check", to_test)
def test_without_z3(e_input, e_check):
    e_new = expr_simp_explicit(e_input)
    print("Original:", e_input, "\tNew:", e_new)
    assert e_new == e_check


@pytest.mark.parametrize("e_input,e_check", to_test)
def test_with_z3(e_input, e_check):
...
@pytest.mark.parametrize("e_input,e_check", to_test_high)
def test_high(e_input, e_check):
...
@pytest.mark.parametrize("e_input,e_check", to_test_conds)
def test_cond(e_input, e_check):
...
def test_match(test, res):
    assert test == res


@pytest.mark.parametrize("test,res", [
    (ExprAssign(ExprMem(a, 32), ExprMem(b, 32)).get_r(True), {a, b, ExprMem(b, 32)}),
    (ExprAssign(ExprMem(a, 32), ExprMem(b, 32)).get_w(), {ExprMem(a, 32)}),
    (ExprAssign(ExprMem(ExprMem(a, 32), 32), ExprMem(b, 32))
     .get_r(True), {a, b, ExprMem(b, 32), ExprMem(a, 32)}),
])
def test_get_rw(test, res):
    assert test == res
