...
def mdis():
    ...
    return machine.dis_engine(cont.bin_stream, loc_db=loc_db)


def test_disasm(mdis):
    first_block = mdis.dis_block(0)
    assert len(first_block.lines) == 5
    print(first_block)


def test_disasm_twice(mdis):
    first_block = mdis.dis_block(0)
...
def test_multiblock(mdis):
    asmcfg = mdis.dis_multiblock(0)
    assert len(asmcfg) == 17


def test_multiblock_twice(mdis):
    first_block = mdis.dis_block(0)
...
@pytest.fixture
def asmcfg_and_last_block(mdis):
    asmcfg = mdis.dis_multiblock(0)
    last_block_loc_key = asmcfg.leaves().pop()
    return mdis, asmcfg, last_block_loc_key


def test_leaves(asmcfg_and_last_block):
    _, asmcfg, _ = asmcfg_and_last_block
...
def test_asmcfg_modification(asmcfg_and_last_block, out_path):
    ...
    # Pending must disappear when adding expected block
...
def test_double_block_split():
    # Check double block split
    data = decode_hex("74097405b8020000007405b803000000b804000000c3")
    cont = Container.from_string(data, loc_db)
    mdis = Machine("x86_32").dis_engine(cont.bin_stream, loc_db=loc_db)
    asmcfg = mdis.dis_multiblock(0)
